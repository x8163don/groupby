...
export default function SettingModal({isOpen, handleOpen, session, onGroupStrategyChange}) {
    ...
    useEffect(() => { setCurGroupStrategy(session.groupStrategy?.constructor?.name) }, [])

    useEffect(() => {
        setCurGroupStrategy(session.groupStrategy?.constructor?.name)
    }, [session.groupStrategy])
    ...
    return <Dialog open={isOpen} handler={handleOpen}>
        <DialogBody>
            ...
            <div className="flex flex-col">
                <Radio name="group" value="PlayCountBaseGroup"
                       defaultChecked={curGroupStrategy === "PlayCountBaseGroup"} onChange={onGroupStrategyHandler}
                       label="依上場次數"/>
                <Radio name="group" value="GenderBaseGroup" defaultChecked={curGroupStrategy === "GenderBaseGroup"}
                       onChange={onGroupStrategyHandler} label="依照性別"/>
            </div>
        </DialogBody>
    </Dialog>
}